...
var resizePizzas = function(size) {
  ...
  // JB - moved windowwidth calculation to calling function and passing it as a parameter.
  // It is being passed from changePizzaSizes below.
  ...
  // JB - Finding a DOM element is expensive and since all the pizzas are
  // in the same container, we only need to get it once so I moved it out
  // of the loop.
  // Moved the windowwidth calculation from determineDx above to here. This value
  // only needs to be calculated once.
  // dx and newwidth are the same for all pizzas so I moved them from inside the
  // the loop as well.
...
function updatePositions() {
  ...
  // JB - Getting the scrollTop value is expensive since we are searching the DOM.
  // Moved this calcution outside the loop since it only needs to be performed once.
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // JB - Moved DOM search out of loop since we only need to search it
  // one time for the movingPizzas1 element.
